...
gridSquareToLatLon = function(grid, obj){
    ...
    if ((grid.length!=2) && (grid.length!=4) && (grid.length!=6)) {
        throw "gridSquareToLatLon: grid must be 2, 4 or 6 chars: "+grid;
    }
    if (/^[A-R][A-R]$/.test(grid)){
        lat = (10*(0.5+grid.charCodeAt(1)-numA))-90;
        lon = (20*(0.5+grid.charCodeAt(0)-numA))-180;
    } else if (/^[A-R][A-R][0-9][0-9]$/.test(grid)){
        lat = lat4(grid)+0.5;
        lon = lon4(grid)+1;
